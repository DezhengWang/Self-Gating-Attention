...
class GatingAttention(nn.Module):
    ...
    def _build_data_logits(self, values):
        """
        稳定的数据驱动 logits：
        1) 能量 = mean(v^2, dim=D)；2) RMS 归一避免顺序偏置；3) per-head 增益；4) 广播到 S。
        返回形状: [B, H, S, F]
        """
        # values: [B, F, H, D] -> [B, H, F, D]
        w = values.transpose(1, 2).contiguous()
        energy = (w ** 2).mean(dim=-1)                      # [B, H, F]
        rms = energy.mean(dim=-1, keepdim=True).sqrt().clamp_min(1e-6)  # [B, H, 1]
        score = energy / rms                                 # [B, H, F]

        gain = F.softplus(self.temp).squeeze(-1)             # [H]
        score = score * gain.unsqueeze(0).unsqueeze(-1)      # [B, H, F]

        data_logits = score.unsqueeze(2) + self.gamma_hs     # [B, H, S, F]
        return data_logits
...
class GatingAttentionWmlp(nn.Module):
    ...
    def __init__(self, alpha_size=None, beta_size=None, attention_dropout=0.1, d_values=64):
        super().__init__()
        assert alpha_size is not None, "alpha_size must be provided"
    ...
    def forward(self, values, w_attn, need_weights=False):
        scale_alpha = 1.0 / sqrt(self.alpha_size[-1])
        # alpha: b, s, h, f

        Pi = torch.softmax(w_attn.mul(scale_alpha), dim=-1)
        Pi = self.attn_dropout(Pi)
        self.Pi = Pi
        values = torch.einsum("blhs,blhd->bshd", self.Pi, values)

        self.attn = torch.softmax(self.alpha.mul(scale_alpha), dim=-1)  # h, s, f
        # values: b, h, f, d
        values = torch.einsum("hsl,blhd->bshd", self.attn, values)
        # values: b, s, h, d
...
class GatingAttentionLayerWmlp(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, enc_in=1,
        ...
        self.w_project = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(self.d_values * num_heads, alpha_size[-1] * num_heads)
        )
    ...
    def forward(self, query, key, value, attn_mask=None, tau=None, delta=None):
        ...
        values = self.value_projection(value).view(B, S, self.n_heads, self.d_values)
        w_attn = self.w_project(value).view(B, S, self.n_heads, -1)
        out, attn = self.inner_attention(values, w_attn, need_weights=self.output_attention)
        output = self.out_projection(out.view(B, -1, self.n_heads * self.d_values))
        # output = out.view(B, -1, self.n_heads * self.d_values)
...
class GatingAttentionWsa(nn.Module):
    ...
    def __init__(self, alpha_size=None, beta_size=None, attention_dropout=0.1, d_values=64):
        ...
        self.denom_bias = nn.Parameter(torch.zeros(h, f, 1))
        self.reset_parameters()
        self.attn_dropout_1 = nn.Dropout(attention_dropout)
        self.attn_dropout_2 = nn.Dropout(attention_dropout)
    ...
    def forward(self, values, w_attn, need_weights=False):
        ...
        # Pi = self.attn_dropout(Pi)
        # self.Pi = Pi
        # values = torch.einsum("bhls,bshd->blhd", self.Pi, values)

        self.attn = torch.softmax((self.attn_dropout_1(w_attn)+self.attn_dropout_2(self.alpha)).mul(scale_alpha), dim=-1)  # h, s, f
        # values: b, h, f, d
        values = torch.einsum("bhsl,blhd->bshd", self.attn, values)
        # values: b, s, h, d
        return values.contiguous(), self.attn if need_weights else None
...
class GatingAttentionLayerWsa(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, tau=None, delta=None):
        ...
        w_attn = torch.einsum("blhe,bshe->bhls", queries, keys)
        out, attn = self.inner_attention(values, w_attn, need_weights=self.output_attention)
        output = self.out_projection(out.view(B, -1, self.n_heads * self.d_values))
        # output = out.view(B, -1, self.n_heads * self.d_values)
...
class GatingAttentionLayerSWT(nn.Module):
    def __init__(self, embed_dim, num_heads, enc_in=1,
                 ...
                 requires_grad=True, wv='db2', m=2, kernel_size=None, d_channel=None, is_sparse=True, beta=0.6, topk_ratio=0.1):
        super().__init__()
        self.cross_attention = cross_attention
        self.output_attention = output_attention
