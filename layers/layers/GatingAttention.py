...
class GatingAttention(nn.Module):
    ...
    def _build_data_logits(self, values):
        ...
        score_exp = score.unsqueeze(2)  # [B,H,1,F]
        data_logits = score_exp * (1 + self.sf_affine_w) + self.sf_affine_b  # [B,H,S,F]
    ...
    def forward(self, values, need_weights=False):
        ...
        std = data_logits.var(dim=-1, unbiased=False, keepdim=True).add(1e-6).sqrt()
